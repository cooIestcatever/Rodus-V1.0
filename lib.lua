function CreateMain(title)
	...
	local function createDrag(object)
		object.MouseEnter:Connect(function()
			checkMouse = true
			mdwn = object.InputBegan:Connect(function(input)
				if input.UserInputType == Enum.UserInputType.MouseButton1 then
					...
					mloo = game:GetService("RunService").RenderStepped:Connect(function()
						if mouseDown then
							local delta = mou.Position - startdrag
							object.Position = UDim2.new(startpos.X.Scale, startpos.X.Offset + delta.X, startpos.Y.Scale, startpos.Y.Offset + delta.Y)
						end
					end)
				end
			end)
			mmoved = object.InputChanged:Connect(function(input)
	...
	function onKeyPress(inputObject, gameProcessedEvent)
		if inputObject.KeyCode == Enum.KeyCode.RightControl then
			if game.CoreGui[title].Top.Visible == false then
				game.CoreGui[title].Top.Visible = true
			else
				game.CoreGui[title].Top.Visible = false
			end
		end
	end
